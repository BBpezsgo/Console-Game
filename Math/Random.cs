namespace ConsoleGame
{
    ...
    {
        public static float Simplex(float x) => (SimplexNoise.Noise.Generate(x) * .5f) + .5f;
        public static float Simplex(Vector pos) => (SimplexNoise.Noise.Generate(pos.X, pos.Y) * .5f) + .5f;
        public static float Simplex(float x, float y) => (SimplexNoise.Noise.Generate(x, y) * .5f) + .5f;
        public static float Simplex(float x, float y, float z) => (SimplexNoise.Noise.Generate(x, y, z) * .5f) + .5f;
    }
    ...
    namespace SimplexNoise
    {
        ...
        /// Based loosely on SimplexNoise1234 by Stefan Gustavson: <see cref="http://staffwww.itn.liu.se/~stegu/aqsis/aqsis-newnoise/"/>
        /// </summary>
