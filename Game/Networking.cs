...
public partial class Game
{
    ...
    {
        ObjectOwner clientOwner = new(client);
        foreach (NetworkEntityComponent item in Scene.NetworkEntityComponents.Components)
        {
            if (item.Owner == clientOwner)
            {
                item.Entity.IsDestroyed = true;
            }
        }
    }
    ...
    void OnDataReceive(Socket sender, byte[] data)
    {
        ...
        {
            OnDataReceive(sender, Message.DeserializeMessage(reader));
        ...
        */
        ...
        if (message is ClientListRequestMessage)
        {
            ...
            connection?.Send(new ClientListMessage()
            {
                ...
                Clients = connection.Clients.ToArray(),
            });
            return;
        }
